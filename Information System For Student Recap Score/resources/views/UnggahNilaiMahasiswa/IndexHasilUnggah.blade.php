...
<!-- Awal Isi Konten -->
<form class="form-horizontal" id="FormUnggah" method="POST" action="{{url('UnggahNilaiMahasiswa')}}">
                  ...
                  <div class="form-group">
                      ...
                      <div class="col-md-4">
                          <div class="input-group">
                              ...
                              <label style="margin-top:7px;" data-toggle="tooltip" data-placement="right" title="Mata Kuliah yang Akan Diunggah"><strong>{{$NamaMk}}</strong></label>
                              <input id = "NamaMk" data-toggle="tooltip" data-placement="right" title="Mata Kuliah yang Akan Diunggah" type="hidden" readonly size="30" value="{{$NamaMk}}" required class="form-control" name="NamaMk" style="width:250px; font-weight:bold; border-radius:2px; color:grey;"/>
                          </div>
                      </div>
                  </div>
                  <div class="form-group">
                      ...
                      <div class="col-md-4">
                          <div class="input-group">
                              <label style="margin-top:7px;" data-toggle="tooltip" data-placement="right" title="Kelas Pararel Mata Kuliah yang Akan Diunggah"><strong>{{$KP}}</strong></label>
                              <input id = "KPMkBuka" type="hidden" data-toggle="tooltip" data-placement="right" title="Kelas Pararel Mata Kuliah yang Akan Diunggah"  readonly size="30" value="{{$KP}}" required class="form-control" name="KPMkBuka" style="width:50px; font-weight:bold; border-radius:2px; color:grey;"/>
                          </div>
                      </div>
                  </div>
          </div>
    ...
    <!-- Akhir Tampil Nilai Lengkap Mahasiswa -->
        ...
        <center>
            <div id="divTombolUnggah" class="form-group">
                <input type='button' id="UnggahNilai" data-toggle="tooltip" data-placement="bottom" title="Klik untuk Mengunggah Nilai" value='Unggah' style='margin-right:10px;' class='btn btn-success'>
            </div>
        </center>
        <!-- Awal Hidden Input -->
        <input type="hidden" id="NPKDosen" name="NPKDosen" value="{{$NPK}}">
        <input type="hidden" id="KodeUnggah" name="KodeUnggah" value="{{$KodeUnggah}}">
...
function DivNilaiLengkapMahasiswa(kodeMk, kpMkBuka, NPKDosen) // Checked V
{
    ...
    // }
    // });
    KodeUnggah = $("#KodeUnggah").val();
    dataTable = $('#divNilaiLengkapMahasiswa').DataTable({
      "processing": true,
      "serverSide": true,
      "bDestroy": true,
      "ajax":{
        url : "jsscript/UnggahNilaiMahasiswaIndexHasilUnggahNilaiMahasiswa.php?kodeMkBuka="+kodeMk+"&kpMkBuka="+kpMkBuka+"&NPK="+NPKDosen+"&KodeUnggah="+KodeUnggah,
...
// Event Unggah Nilai
$('#UnggahNilai').on('click', function() // Checked V
{
    $.ajax({
    url: "jsscript/UnggahNilaiMahasiswaIndexHasilUnggahCheckValidAdmik.php?KodeMkBuka="+kodeMk+"&KPMkBuka="+kpMkBuka+"&KodeUnggah="+KodeUnggah,
    context: document.body,
    success: function(responseText) {
        // alert(responseText);
        if (responseText == 1)
        {
            alertify.confirm("Apakah Anda Yakin Ingin Mengunggah Nilai Mahasiswa Mata Kuliah {{$NamaMk}} KP {{$KP}} ?",
            function(){
                $('#FormUnggah').submit();
                alertify.success('Nilai Mahasiswa Mata Kuliah {{$NamaMk}} KP {{$KP}} Telah Diunggah');
            },
            function(){
                alertify.error('Proses Unggah Dibatalkan');
            });
        }
        else
        {
            alertify.alert("Nilai Telah Divalidasi Oleh Admik, Anda Tidak Dapat Mengunggah Nilai !", function(){
                // alertify.message('OK');
            });
            noty({text: "Nilai Telah Divalidasi Oleh Admik, Anda Tidak Dapat Mengunggah Nilai !", layout: 'topRight', type: 'error'});
        }
    }
    });
});
...
$(document).on('click', '#divNilaiLengkapMahasiswa tbody tr td.details-control', function() // Checked V
{
    ...
    else
    {
      ...
      $.ajax({
      url: "jsscript/UnggahNilaiMahasiswaTampilDetailNilai.php?KodeSubString="+datarow+"&KodeMkBuka="+kodeMk+"&KPMkBuka="+kpMkBuka+"&KodeUnggah="+KodeUnggah,
      context: document.body,
      success: function(responseText) {
          // alert(responseText);
